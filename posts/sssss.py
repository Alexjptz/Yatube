...
class TestPostsApp(TestCase):
    ...
    def test_user_new_post(self):
        ...
        #  post = response.context['paginator'].object_list.first()
        #  не работает, response.context - пуст
    ...
    def test_post_edit(self):
        ...
        for page in pages:
            with self.subTest(page=page):
                cache.clear()
                page_response = self.client_auth.get(page)
                print(page_response.status_code)
                print(page_response.context['paginator'].object_list)
                print(page)
                con = page_response.context
                print(type(con))
                print(page_response.context['paginator'])
                print('=============================')
                self.assertEqual(
                ...
                if page_response.context['paginator']:
                    # self.assertEqual(
                    #     len(page_response.context['paginator'].object_list.count),
                    #     1
                    # )
                    self.assertIn(
                        post,
                        page_response.context['paginator'].object_list,
                        msg=f'Пост отсутствует в {page} Paginator, проверь'
                    )
                else:
                    self.assertEquals(
                        post,
                        page_response.context['post'],
                        msg=f'Пост отсутствует в context {page}'
                    )
